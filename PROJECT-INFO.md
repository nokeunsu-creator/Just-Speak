...
- **난이도 라벨 + 그룹 헤더** — 초급/중급/고급, 5개 그룹(일상·업무·이동·여가·생활)
- **카테고리별 학습 가이드** — 상황 + 핵심표현 5개 + 문법 포인트 + 쉐도잉 4단계
- **줄별 단어 팁(ⓘ)** — 어려운 표현 25개에 단어 풀이
- **즐겨찾기 ⭐** — 학습 화면에서 줄별 토글, 즐겨찾기 탭에서 모아보기
- **음성 녹음 비교 🎤** — 마이크로 따라말하기 녹음 → 재생 / 비교 재생
- **검색 🔍** — 카테고리·문장(영/한) 통합 검색
- **학습 통계** — 총 학습 문장, 즐겨찾기 수, 연속 학습 일수(streak), 최근 7일 활동 그래프
