...
export function Welcome() {
  ...
  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4">
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-800">
        ...
        <section className="mb-5 rounded-xl bg-amber-50 p-3 dark:bg-amber-900/30">
          <h3 className="mb-1 text-xs font-bold text-amber-700 dark:text-amber-300">
            {t('welcomeBeginnerHeading', lang)}
          </h3>
          <p className="mb-2 text-xs text-amber-800/90 dark:text-amber-200/90">
            {t('welcomeBeginnerHint', lang)}
          </p>
          <div className="flex flex-wrap gap-1">
            {picks.map((p) => (
              <span key={p.id} className="rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-amber-700 shadow-sm dark:bg-slate-800 dark:text-amber-300">
                {p.emoji} {p.name}
              </span>
            ))}
          </div>
